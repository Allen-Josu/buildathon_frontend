...
export default function PageLayout({ title, entity }) {
		...
		const fetchUserLikedEntities = async () => {
			...
			try {
				const response = await axios.get(
					`${BASE_URL}/users?entity=users&studentId=${user.studentId}&entityType=single&entityId=${user.entityId}`
				);
	...
	// Set course options and active subject
	useEffect(() => {
		const fetchData = async () => {
			try {
				const courses = [...new Set(responseData.map((item) => item.course))]
					.map((course) => ({
						label: course,
						value: course,
					}));
			...
			<Flex gap="middle" wrap="wrap" className="min-h-screen">
				<Layout style={layoutStyle}>
					...
					<div className="flex flex-col md:flex-row md:justify-between md:items-center w-full px-4 md:px-8 lg:px-20 py-4 md:h-20 text-white bg-[#27272a] border-b-2 border-[#393939] gap-4">
						...
						<div className="flex flex-col md:flex-row gap-4 md:w-auto w-full">
							{user && (
								<Button
									type="primary"
									style={{ background: "#6d28d9", width: "200px", border: "none" }}
					...
					<Layout className="relative">
						...
						>
							<ChevronRight
								className={`text-white transform transition-transform ${isMobileMenuOpen ? "rotate-180" : ""
									}`}
							/>
						</button>
							...
							>
								...
								{loading ? (
									...
									<div className="space-y-4 mt-6 px-10 py-3">
										{filteredData.sort((a, b) => b.likes - a.likes).map((item) => (
														...
														</div>
														{
															user && <>
																<div className="flex items-center gap-2">
																	...
																	<Heart
																		onClick={(e) => handleLikeClicked(e, item.entityId, item.likes)}
																		fill={isEntityLikedByUser(item.entityId, userLikedEntity) ? "#c1c3c8" : "none"}
